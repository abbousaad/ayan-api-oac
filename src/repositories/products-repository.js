...
const getAllProducts = async ({ storeId } = {}) => {
  ...
  const query = storeId
    ? 'SELECT id, store_id, name, description, price, stock, unit FROM products WHERE store_id = $1 ORDER BY created_at ASC'
    : 'SELECT id, store_id, name, description, price, stock, unit FROM products ORDER BY created_at ASC';
...
const getProductById = async (id) => {
  ...
  const result = await runQuery(
    'SELECT id, store_id, name, description, price, stock, unit FROM products WHERE id = $1 LIMIT 1',
    [id]
  );
...
const addProduct = async ({ storeId, name, description = '', price, stock, unit = 'unit' }) => {
  ...
  const result = await runQuery(
    `INSERT INTO products (id, store_id, name, description, price, stock, unit)
     ...
     RETURNING id, store_id, name, description, price, stock, unit`,
    [id, storeId, name, description, Number(price), Number(stock), unit]
  );
...
const editProduct = async (id, changes) => {
  ...
  const result = await runQuery(
     `UPDATE products
     SET store_id = $2, name = $3, description = $4, price = $5, stock = $6, unit = $7, updated_at = NOW()
     WHERE id = $1
     RETURNING id, store_id, name, description, price, stock, unit`,
    [id, next.storeId, next.name, next.description, next.price, next.stock, next.unit]
  );
